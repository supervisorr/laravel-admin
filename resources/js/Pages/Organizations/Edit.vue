<template>
  <div>
    ...
    <h2 class="mt-12 font-bold text-2xl">Posts</h2>
    <div class="mt-6 bg-white rounded shadow overflow-x-auto">
      <table class="w-full whitespace-nowrap">
        ...
        <tr v-for="post in organization.posts" :key="post.id" class="hover:bg-gray-100 focus-within:bg-gray-100">
          <td class="border-t">
            <inertia-link class="px-6 py-4 flex items-center focus:text-indigo-500" :href="route('posts.edit', post.id)">
              {{ post.name }}
              <icon v-if="post.deleted_at" name="trash" class="flex-shrink-0 w-3 h-3 fill-gray-400 ml-2" />
            </inertia-link>
          </td>
          <td class="border-t">
            <inertia-link class="px-6 py-4 flex items-center" :href="route('posts.edit', post.id)" tabindex="-1">
              {{ post.city }}
            </inertia-link>
          </td>
          <td class="border-t">
        ...
        <tr v-if="organization.posts.length === 0">
